...
//This Function allow the user to choose one of the 'Working" tasks
void PrintWorkingTask(){
    int c=0;
    cout<<"****************************"<<endl;
...
string convert(string userTask){
    string content, line;
    fstream File;
        File.open(File_Name, ios::in); // Read mode
...
//This function will alow the user to success his tasks.
void check(){
    while(true){
        system("cls");
        PrintWorkingTask();//Calling the 'PrintWorkingTask' function
        cout<<"\nPlease enter the completed task: ";
        int userChoice=-9999;
        cin>>userChoice;
        string userTask=Load_The_Working_Tasks_From_File()[userChoice-1];
        string content=convert(userTask);
        fstream File;
